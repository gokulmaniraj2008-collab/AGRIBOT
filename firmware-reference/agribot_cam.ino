...
const char* SUPABASE_SERVICE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh2bmFzaXBwd2FkenlnbmFvZHBwIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NTkyODc0MywiZXhwIjoyMDkxNTA0NzQzfQ.iNgdptmdbDdq94f_QNVFIcRD3Ny8eb9tVp2q1nMGbX8";  // your key here
const char* SUPABASE_BUCKET      = "robot-images";
...
bool uploadToStorage(uint8_t* data, size_t len, String filename) {
  ...
  if (!success) Serial.printf("Upload failed, HTTP %d: %s\n", code, http.getString().c_str());
  else Serial.println("Image uploaded: " + filename);
  http.end();
  return success;
}

void insertImageRecord(String storagePath) {
  ...
  HTTPClient http;
  http.begin(String(SUPABASE_URL) + "/rest/v1/robot_images");
  http.addHeader("apikey", SUPABASE_SERVICE_KEY);
  http.addHeader("Authorization", String("Bearer ") + SUPABASE_SERVICE_KEY);
  http.addHeader("Content-Type", "application/json");
  int code = http.POST(payload);
  if (code < 200 || code >= 300) Serial.printf("Record insert failed, HTTP %d: %s\n", code, http.getString().c_str());
  http.end();
}
