...
// 4-wheel mirrored chassis motor polarity:
// LEFT side  = IN1/IN2
// RIGHT side = IN3/IN4 (electrically reversed because the motors are mirrored)
// FORWARD:  LEFT HIGH/LOW,  RIGHT LOW/HIGH
// BACKWARD: LEFT LOW/HIGH,  RIGHT HIGH/LOW
  ...
  while (millis() - navStart < GPS_NAV_TIMEOUT_MS) {
    ...
    if (gps.course.isValid() && gps.course.age() < 2000) {
      ...
      if (abs(headingError) > GPS_HEADING_TOLERANCE_DEG) {
        if (headingError > 0) {
          digitalWrite(IN1_PIN,HIGH); digitalWrite(IN2_PIN,LOW);
          digitalWrite(IN3_PIN,HIGH); digitalWrite(IN4_PIN,LOW); motorState = "right";
        } else {
          digitalWrite(IN1_PIN,LOW); digitalWrite(IN2_PIN,HIGH);
          digitalWrite(IN3_PIN,LOW); digitalWrite(IN4_PIN,HIGH); motorState = "left";
        }
        applySpeed();
...
void executeCommand(const String& command, int value) {
  if (command == "forward") {
    digitalWrite(IN1_PIN,HIGH); digitalWrite(IN2_PIN,LOW); digitalWrite(IN3_PIN,LOW); digitalWrite(IN4_PIN,HIGH); motorState="forward";
    ...
    digitalWrite(IN1_PIN,LOW); digitalWrite(IN2_PIN,HIGH); digitalWrite(IN3_PIN,HIGH); digitalWrite(IN4_PIN,LOW); motorState="backward";
    applySpeed();  // BUGFIX: same as forward, above.
  } else if (command == "left") {
    digitalWrite(IN1_PIN,LOW); digitalWrite(IN2_PIN,HIGH); digitalWrite(IN3_PIN,LOW); digitalWrite(IN4_PIN,HIGH); motorState="left";
    applySpeed();  // BUGFIX: same as forward, above.
  } else if (command == "right") {
    digitalWrite(IN1_PIN,HIGH); digitalWrite(IN2_PIN,LOW); digitalWrite(IN3_PIN,HIGH); digitalWrite(IN4_PIN,LOW); motorState="right";
    applySpeed();  // BUGFIX: same as forward, above.
