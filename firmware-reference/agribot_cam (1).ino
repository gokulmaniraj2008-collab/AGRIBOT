...
// Local MJPEG stream server. Only reachable from devices on the same WiFi
// as the ESP32-CAM (see LIVE STREAM note near pushCamStatus below) — this
// does NOT make the camera reachable from the public agribot.website.
WebServer camServer(80);

// Explicit forward declarations. The Arduino IDE auto-generates its own
// prototypes for every function in the sketch, and if a function's
// definition includes a default argument (camLogEvent's is_error below),
// the auto-generated prototype duplicates that default — which C++
// forbids (you get "redefinition of default argument", plus confusing
// cascading errors like an "ambiguous" call or a bogus "no matching
// member function" elsewhere). Declaring the prototype ourselves here
// (with the default) stops Arduino from generating a conflicting one;
// the default is then removed from the definition further down.
void camLogEvent(const String& message, bool is_error = false);
...
const char* GEMINI_API_KEY    = "AQ.Ab8RN6JKosHJ62TwQ-gKTh4pWMcYGqWM4QTnSiQDcoGpFUWbXg"; // see SECURITY WARNING above
...
// Writes a human-readable note into device_messages (origin: "esp32") —
// the free-text "communication" channel the website's Message Log
// section reads, same table/shape agribot_main.ino's pushMessage()
// writes to. Kept selective on purpose: only the handful of events
// worth a human's attention (came online, hard failure, a check
// failed) — NOT the granular step-by-step, which belongs in
// robot_logs via camLogEvent() instead. Uses DEVICE_ID, matching the
// device_messages anon insert policy (robot_id='agribot-01' AND
// origin='esp32') and the same identity convention as everywhere else
// in this file.
void camPushMessage(const String& message, const String& level) {
  Serial.printf("[camera say] (%s) %s\n", level.c_str(), message.c_str());
  ...
  if (code < 200 || code >= 300) {
    Serial.printf("device_messages write failed: HTTP %d\n", code);
  }
  http.end();
}

// Writes a CAMERA event into robot_logs, the same structured activity-log
// table agribot_main.ino's logEvent() writes to — the website's /logs
// page already renders event_type "CAMERA" with its own icon (see
// EVENT_META in logs-client.tsx), so no website changes are needed.
//
// IMPORTANT: uses robot_id = DEVICE_ID ("agribot-01"), NOT CAM_ROBOT_ID.
// robot_logs' anon insert policy only allows robot_id = 'agribot-01' —
// same identity convention as robot_images/robot_commands elsewhere in
// this file (see IDENTITY NOTE at the top). Using CAM_ROBOT_ID here
// would get silently rejected by RLS, the same bug that existed on
// robot_status before it was fixed.
//
// Fire-and-forget: failures only print to Serial, matching every other
// network call in this file. is_error just changes how the website's
// /logs page colors the entry (warning tone for CAMERA + error kind);
// it doesn't change where the row is written.
void camLogEvent(const String& message, bool is_error) {
  Serial.printf("[CAMERA]%s %s\n", is_error ? " [ERROR]" : "", message.c_str());

  if (WiFi.status() != WL_CONNECTED) return;

  StaticJsonDocument<256> doc;
  doc["robot_id"] = DEVICE_ID;
  doc["event_type"] = "CAMERA";
  doc["message"] = is_error ? ("ERROR: " + message) : message;
  String payload; serializeJson(doc, payload);

  String url = String(SUPABASE_URL) + "/rest/v1/robot_logs";
  HTTPClient http;
  if (!http.begin(url)) return;
  http.addHeader("apikey", SUPABASE_ANON_KEY);
  http.addHeader("Authorization", String("Bearer ") + SUPABASE_ANON_KEY);
  ...
  int code = http.POST(payload);
  if (code < 200 || code >= 300) {
    Serial.printf("robot_logs write failed: HTTP %d\n", code);
  }
  http.end();
}
...
void pushCamStatus() {
  ...
  // camera_ip lets the website build the local MJPEG stream URL
  // (http://camera_ip/stream). This only resolves for clients on the
  // same WiFi network as the camera — see LIVE STREAM note above.
  StaticJsonDocument<192> doc;
  doc["robot_id"] = CAM_ROBOT_ID;
  doc["name"] = "AgriBot Camera";
  doc["online"] = true;
  doc["camera_ip"] = WiFi.localIP().toString();
  String payload; serializeJson(doc, payload);
...
// ---------- LIVE STREAM (local WiFi only) ----------
// Separate from the Supabase snapshot/classification path above.
// http://<camera_ip>/       -> test page with embedded <img> viewer
// http://<camera_ip>/stream -> raw MJPEG multipart stream (~10 FPS,
//                              capped by the 100ms delay below)
// Only reachable while the viewing device is on the same WiFi as this
// board. Does NOT go through Supabase and is NOT reachable from the
// public agribot.website unless you're on that same local network.
void handleCamRoot() {
  String html =
    "<!DOCTYPE html><html><head>"
    "<meta name='viewport' content='width=device-width,initial-scale=1'>"
    "<title>AGRIBOT Camera</title></head><body>"
    "<h2>AGRIBOT ESP32-CAM</h2>"
...
void handleCamStream() {
  ...
  // Using client.write(buf, len) throughout instead of client.print()/
  // printf() — some ESP32 core versions resolve camServer.client()'s
  // print()/printf() overloads ambiguously (or not at all), causing
  // "no matching member function" build errors. write(const uint8_t*,
  // size_t) is the one method every Client/Print-derived class reliably
  // has, so it avoids that overload-resolution problem entirely.
  static const char* headerStr =
    "HTTP/1.1 200 OK\r\n"
    ...
    "\r\n";
  client.write((const uint8_t*)headerStr, strlen(headerStr));

  while (client.connected()) {
    camera_fb_t* fb = esp_camera_fb_get();
    if (!fb) {
      Serial.println("Live stream capture failed");
      break;
    }

    char frameHeader[64];
    int frameHeaderLen = snprintf(frameHeader, sizeof(frameHeader),
      "--frame\r\n"
      "Content-Type: image/jpeg\r\n"
      "Content-Length: %u\r\n"
      "\r\n",
      (unsigned)fb->len);
    client.write((const uint8_t*)frameHeader, frameHeaderLen);
    client.write(fb->buf, fb->len);
    client.write((const uint8_t*)"\r\n", 2);

    esp_camera_fb_return(fb);

    // Also let the heartbeat/command-poll/snapshot-upload loop() run —
    // this handler blocks loop() while a client is connected, so those
    // won't fire again until streaming stops. Fine for short viewing
    // sessions; keep that in mind if you leave a stream open for a while.
    delay(100);
  }
}
...
bool uploadFrame(camera_fb_t* fb) {
  ...
  if (ok) {
    ...
    Serial.println(resp);
    camLogEvent("Supabase upload failed", true);
  }
...
void pollForCameraCheckCommand() {
  ...
  camLogEvent("camera_check received");

  camera_fb_t* fb = esp_camera_fb_get();
  if (!fb) {
    Serial.println("Camera capture failed for camera_check.");
    camLogEvent("Camera capture failed", true);
    camPushMessage("Camera capture failed during patrol check", "warning");
    return;
  }
  camLogEvent("Image captured");
  ...
  if (succeeded) {
    camLogEvent(String("AI result: ") + (isPlant ? "PLANT" : "NOT_PLANT"));
  } else {
    camLogEvent("Gemini request failed", true);
    camPushMessage("AI classification failed — defaulting to PLANT", "warning");
  }
  ...
  camLogEvent("Classification saved");
...
void setup() {
  ...
  delay(1000);
  Serial.println("\nESP32-CAM starting...");
  camLogEvent("Camera booting");

  if (!initCamera()) {
    Serial.println("Camera init failed. Halting.");
    camLogEvent("Camera init failed", true);
    camPushMessage("Camera init failed — halting", "error");
  ...
  connectWiFi();
  if (WiFi.status() == WL_CONNECTED) {
    camLogEvent("Wi-Fi connected");
    camPushMessage("Camera online", "info");
    ...
    camServer.on("/", HTTP_GET, handleCamRoot);
    camServer.on("/stream", HTTP_GET, handleCamStream);
    camServer.begin();
    Serial.println("Camera web server started");
    Serial.print("Camera URL: http://");
    Serial.println(WiFi.localIP());
    camLogEvent("Live stream started");
  } else {
    camLogEvent("Live stream unavailable", true);
  }
}
