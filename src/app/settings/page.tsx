    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          ...
          setTempMinInput(String(data.target_temp_min));
          setTempMaxInput(String(data.target_temp_max));
        }
      });
  }, [supabase]);
    ...
    try {
      ...
      if (res.ok) {
        setStatus((prev) => {
          ...
          if (command === "set_ventilation_auto_on") return { ...prev, ventilation_auto: true };
          if (command === "set_ventilation_auto_off") return { ...prev, ventilation_auto: false };
          if (command === "set_irrigation_threshold" && value != null)
            return { ...prev, irrigation_threshold: value };
          if (command === "set_target_temp_min" && value != null)
            return { ...prev, target_temp_min: value };
          if (command === "set_target_temp_max" && value != null)
            return { ...prev, target_temp_max: value };
          return prev;
        });
      }
    } finally {
      ...
      <>
        ...
        <div className="mt-6">
          ...
          <div className="space-y-2.5">
            ...
            <Card className="p-3.5">
              <div className="flex items-center gap-3">
                <IconTile icon={Wind} size={32} color="#0ea5e9" />
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-foreground dark:text-gray-100">
                    Auto Ventilation
                  </p>
                  <p className="text-xs text-muted dark:text-gray-400">
                    Runs the vent fan to keep temperature in range
                  </p>
                </div>
                <Switch
                  checked={!!status?.ventilation_auto}
                  disabled={sending != null}
                  onChange={() =>
                    sendCommand(
                      status?.ventilation_auto
              ...
              <div className="mt-3 flex items-center gap-2 border-t border-border pt-3 dark:border-gray-800">
                ...
                <label className="text-xs font-medium text-muted dark:text-gray-400">Range</label>
                <input
                  type="number"
                  value={tempMinInput}
                  onChange={(e) => setTempMinInput(e.target.value)}
                  onBlur={() => {
                    const v = Number(tempMinInput);
                    if (!Number.isNaN(v) && v !== status?.target_temp_min) {
                      sendCommand("set_target_temp_min", v);
                    }
                  }}
                  className="w-14 rounded-lg border border-border bg-white px-2 py-1 text-xs font-semibold text-foreground outline-none focus:border-primary dark:border-gray-800 dark:bg-gray-900 dark:text-gray-100"
                />
                <span className="text-xs text-muted dark:text-gray-400">–</span>
                <input
                  type="number"
                  value={tempMaxInput}
                  onChange={(e) => setTempMaxInput(e.target.value)}
                  onBlur={() => {
                    const v = Number(tempMaxInput);
                    if (!Number.isNaN(v) && v !== status?.target_temp_max) {
                      sendCommand("set_target_temp_max", v);
                    }
                  }}
                  className="w-14 rounded-lg border border-border bg-white px-2 py-1 text-xs font-semibold text-foreground outline-none focus:border-primary dark:border-gray-800 dark:bg-gray-900 dark:text-gray-100"
                />
                <span className="text-xs text-muted dark:text-gray-400">°C</span>
              </div>
            </Card>
          </div>
        </div>
