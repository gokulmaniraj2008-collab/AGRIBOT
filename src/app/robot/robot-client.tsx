  ...
  return (
    <DashboardShell title="Robot Control" subtitle="AgriBot AI — Unit 01" online={active}>
      <>
        ...
        <section className="mt-4 rounded-2xl p-4 shadow-sm" style={tint(SECTION_TINTS.climate)}>
          <div className="mb-3 flex items-center justify-between">
            <span className="text-sm font-medium text-foreground dark:text-gray-100">Climate Control</span>
            <span className="text-xs text-muted dark:text-gray-400">
              Target {status?.target_temp_min ?? "—"}–{status?.target_temp_max ?? "—"}°C
            </span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <ToggleButton
              ...
              label="Heater"
              on={status?.heater_status ?? false}
              color={SECTION_TINTS.climate}
              onClick={() => sendCommand(status?.heater_status ? "heater_off" : "heater_on")}
              disabled={sending === "heater_on" || sending === "heater_off"}
            />
            <ToggleButton
              icon={<Snowflake className="h-3.5 w-3.5" />}
              label="Cooler"
              on={status?.cooler_status ?? false}
              color="#0891b2"
              onClick={() => sendCommand(status?.cooler_status ? "cooler_off" : "cooler_on")}
              disabled={sending === "cooler_on" || sending === "cooler_off"}
            />
          </div>
        </section>
        ...
        <section className="mt-4 rounded-2xl p-4 shadow-sm" style={tint(SECTION_TINTS.ventilation)}>
          <div className="mb-3 flex items-center justify-between">
            <span className="text-sm font-medium text-foreground dark:text-gray-100">Ventilation & Fans</span>
            <span className="text-xs capitalize text-muted dark:text-gray-400">
              {status?.vent_fan_status ? "Running" : "Stopped"}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <ToggleButton
              ...
              label="Fan"
              on={status?.vent_fan_status ?? false}
              color={SECTION_TINTS.ventilation}
              onClick={() => sendCommand(status?.vent_fan_status ? "vent_off" : "vent_on")}
              disabled={sending === "vent_on" || sending === "vent_off"}
            />
            <ToggleButton
              icon={<Power className="h-3.5 w-3.5" />}
              label="Auto Ventilation"
              on={status?.ventilation_auto ?? false}
              color={SECTION_TINTS.ventilation}
              onClick={() =>
                sendCommand(status?.ventilation_auto ? "set_ventilation_auto_off" : "set_ventilation_auto_on")
              }
              disabled={sending === "set_ventilation_auto_on" || sending === "set_ventilation_auto_off"}
            />
          </div>
        </section>
      </>
