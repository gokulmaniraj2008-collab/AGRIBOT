...
import { getDemoTelemetry, getDemoSensorHistory, getDemoAlerts, getDemoAIInsight } from "@/lib/demo-data";
      ...
      <div className="px-5 lg:px-8 py-6 lg:py-8 max-w-7xl">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="mt-1 text-sm text-text-secondary">{telemetry.robotId} · Last update {telemetry.lastSeenSeconds}s ago</p>
          </div>
          <DemoBadge />
        </div>

        <div className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Farm Health" value={`${telemetry.farmHealthPct}%`} icon={HeartPulse} tone="success" />
          <StatCard label="Soil Moisture" value={`${telemetry.soilMoisturePct}%`} icon={Droplet} tone="default" />
          <StatCard label="Battery" value={`${telemetry.batteryPct}%`} icon={BatteryMedium} tone={telemetry.batteryPct < 20 ? "danger" : "default"} />
